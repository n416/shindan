...
tmpl,direction,job,cid=open("scripts/_idle_probe.txt").read().split()
t0=time.time()
while True:
...
for a in d["animations"]:
    if "idle" in (a.get("animation_type") or "").lower() or "breath" in (a.get("animation_type") or "").lower():
        for x in a["directions"]:
            if x["direction"]==direction: frames=x["frames"]  # 最新優先で上書き
ims=[fetch(u) for u in frames]
print("frames:",len(ims),"dir:",direction,"size:",ims[0].size,flush=True)
os.makedirs("public/_debug",exist_ok=True)
pal=[]
for f in ims:
    al=f.getchannel("A"); p=f.convert("RGB").quantize(colors=255); p.paste(255,al.point(lambda x:255 if x<=128 else 0)); pal.append(p)
pal[0].save("public/_debug/idle_front.gif",save_all=True,append_images=pal[1:],duration=130,loop=0,transparency=255,disposal=2)
# コンタクトシート
w,h=ims[0].size; sheet=Image.new("RGBA",(w*len(ims),h),(25,25,32,255))
for i,im in enumerate(ims): sheet.paste(im,(i*w,0),im)
sheet.save("public/_debug/idle_front_sheet.png")
print("wrote /_debug/idle_front.gif",flush=True)
